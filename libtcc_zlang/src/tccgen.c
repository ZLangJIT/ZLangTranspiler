...
static const char * prefix_stack[200];
const char ** prefix_stack_ptr = prefix_stack;

void push_prefix(const char * prefix) {
    if (prefix_stack_ptr == NULL) {
      prefix_stack_ptr = prefix_stack;
    }
    *(prefix_stack_ptr++) = prefix;
}

int needs_prefix() { return prefix_stack_ptr != NULL ? 1 : 0; }

void pop_prefix() {
    if (prefix_stack_ptr == NULL) tcc_error("unbalanced prefix push/pop");
    *(prefix_stack_ptr--) = NULL;
    if (prefix_stack_ptr == prefix_stack) prefix_stack_ptr = NULL;
}

unsigned int prefix_tok(unsigned int tok) {
    CString cstr;
    cstr_new(&cstr);
    {
      const char ** begin = prefix_stack;
      const char ** end = prefix_stack_ptr;
      for (; begin != end; begin++) {
          cstr_cat(&cstr, *begin, strlen(*begin));
          cstr_cat(&cstr, "::", 3);
      }
    }
    const char * k = get_tok_str(tok, NULL);
    cstr_cat(&cstr, k, strlen(k));
    cstr_cat(&cstr, "", 1); // append '\0'
    unsigned int h = TOK_HASH_INIT;
    for(uint8_t i = 0; i < cstr.size; i++) {
      h = TOK_HASH_FUNC(h, ((unsigned char *)cstr.data)[i]);
    }
    h &= (TOK_HASH_SIZE - 1);
    ...
    for(;;) {
        ...
        if (!ts)
            break;
        if (ts->len == cstr.size && !memcmp(ts->str, cstr.data, cstr.size))
            goto token_found;
        pts = &(ts->hash_next);
    }
    ts = tok_alloc_new(pts, (char *) cstr.data, cstr.size);
    token_found: ;
    cstr_free(&cstr);
    return ts->tok;
}
...
#define STRIP_FLAG(sym, flag) ((sym) & flag ? (sym) & ~flag : (sym))
...
//#define BF_DEBUG
    ...
    for (f = type->ref->next; f; f = f->next) {
        if (f->type.t & VT_FUNC) {
          if (f->fwd) {
              tcc_warning("SYM FUNC field '%s' forwards to global symbol '%s'", get_tok_str(f->v & ~(SYM_FIELD|VT_FUNC), NULL), get_tok_str(f->fwd->v, NULL));
          } else {
              tcc_warning("SYM FUNC field '%s'", get_tok_str(f->v & ~(SYM_FIELD|VT_FUNC), NULL));
          }
          //f = f->next;
              // if (f->fwd)
              //     tcc_warning("next function field '%s' must not have a forward", get_tok_str(f->v & ~(SYM_FIELD|VT_FUNC), NULL));
          continue;
        } else {
          tcc_warning("SYM field '%s'", get_tok_str(f->v & ~SYM_FIELD, NULL));
        }
	...
	printf("set field %s offset %-2d size %-2d align %-2d",
	       get_tok_str(f->v, NULL), offset, size, align);
	if (f->type.t & VT_BITFIELD) {
...
static void struct_decl(CType *type, int u)
{
    ...
    if (tok != '{') {
        ...
        if (v < TOK_IDENT) {
          if (v == TOK_STRUCT)
              tcc_error("struct name");
          if (v == TOK_UNION)
              tcc_error("union name");
          if (v == TOK_ENUM)
              tcc_error("enum name");
          if (v == TOK_CLASS)
              tcc_error("class name");
            expect("struct/class/union/enum name");
        }
        if (v == TOK_STRUCT)
            tcc_error("'struct struct' is not allowed");
        else if (v == TOK_UNION)
            tcc_error("'struct union' is not allowed");
        else if (v == TOK_ENUM)
            tcc_error("'struct enum' is not allowed");
        else if (v == TOK_CLASS)
            tcc_error("'struct class' is not allowed");
        s = struct_find(v);
...
// C++
/* enum/struct/union declaration. u is VT_ENUM/VT_STRUCT/VT_UNION */
static void struct_or_class_decl(CType *type, int u)
{
    int v, c, size, align, flexible;
    int bit_size, bsize, bt;
    Sym *s, *ss, **ps;
    AttributeDef ad, ad1;
    CType type1, btype;
    unsigned int old_ptr = tcc_state->hierarchy_stack_pointer;
    ...
    if (tok != '{') {
        ...
        if (v == TOK_STRUCT)
            tcc_error("'class struct' is not allowed");
        else if (v == TOK_UNION)
            tcc_error("'class union' is not allowed");
        else if (v == TOK_ENUM)
            tcc_error("'class enum' is not allowed");
        else if (v == TOK_CLASS)
            tcc_error("'class class' is not allowed");
        s = struct_find(v);
        if (s && (s->sym_scope == local_scope || tok != '{')) {
            if (u == s->type.t)
                goto do_decl;
    ...
    s->r = 0; /* default alignment is zero as gcc */
do_decl:
    if (s) tcc_state->hierarchy_stack[tcc_state->hierarchy_stack_pointer++] = s;
    type->t = s->type.t;
    type->ref = s;
    s->type.ref = s;
			...
			if (tok != ';')
			    ...
			    if ((u == VT_STRUCT) && (type1.t & VT_ARRAY) && c)
			        ...
			        tcc_error("field '%s' has incomplete type",
                        ...
                        if ((type1.t & VT_BTYPE) == VT_VOID ||
                            ((type1.t & VT_STORAGE) && !(type1.t & VT_STATIC)))
                            tcc_error("invalid type for '%s'", 
                                  get_tok_str(v, NULL));
                      }
                      if ((type1.t & VT_BTYPE) == VT_FUNC) { // handle function
                          int l = VT_CONST;
                          Sym * sym;
                          if ((type1.t & VT_BTYPE) == VT_FUNC) {
                              if ((type1.t & VT_STATIC) && (l == VT_LOCAL))
                          ...
                          type1.t &= ~VT_EXTERN;
                          /* put function symbol */
                          ss = sym_push(v | SYM_FIELD | VT_FUNC, &type1, 0, 0);
                          ss->a = ad1.a;
                          *ps = ss;
                          ps = &ss->next;
                          sym = ss;
                          tcc_state->hierarchy_stack[tcc_state->hierarchy_stack_pointer++] = sym;
                          CString cstr;
                          cstr_new(&cstr);
                          for(uint8_t i = 0; i < tcc_state->hierarchy_stack_pointer; i++) {
                              char buf[500];
                              int16_t bi = 0;
                              type_to_str(buf, sizeof(buf), &tcc_state->hierarchy_stack[i]->type, NULL);
                              tcc_warning("hashing type '%s'", buf);
                              if (i != 0) cstr_cat(&cstr, ".", 1);
                              cstr_cat(&cstr, buf, strlen(buf));
                          }
                          const char * tok_str = get_tok_str(v, NULL);
                          tcc_warning("hashing name '%s'", tok_str);
                          cstr_cat(&cstr, ".", 1);
                          cstr_cat(&cstr, tok_str, strlen(tok_str));
                          cstr_cat(&cstr, "", 1); // append '\0'
                          tcc_state->hierarchy_stack_pointer--;
                          unsigned int h = TOK_HASH_INIT;
                          tcc_warning("hashing type '%s'", (unsigned char *)cstr.data);
                          for(uint8_t i = 0; i < cstr.size; i++) {
                            h = TOK_HASH_FUNC(h, ((unsigned char *)cstr.data)[i]);
                          }
                          h &= (TOK_HASH_SIZE - 1);
                          ...
                          token_found: ;
                          cstr_free(&cstr);
                          sym->fwd = external_sym(ts->tok, &type1, 0, &ad);
                          if (tok == ';' || tok == TOK_EOF) {
                              skip(';');
                              break;
                          }
                          if (tok == '{') {
                              ...
                              if (sym->fwd->type.t & VT_INLINE) {
                                  ...
                                  if (!cur_text_section)
                                      cur_text_section = text_section;
                                  gen_function(sym->fwd);
                              }
                              //next();
                              break;
                          } else {
                            tcc_error("expected ';' or '{'");
                          }
                      }
                      if (tok == ':') {
                          next();
    ...
    tcc_state->hierarchy_stack_pointer = old_ptr;
}
...
static int parse_btype(CType *type, AttributeDef *ad)
{
    ...
    while(1) {
        ...
        case TOK_CLASS:
        {
            struct_or_class_decl(&type1, VT_STRUCT);
            // next(); // skip 'class'
            // if (tok != '{') {
            //   CType c;
            //   Sym * s = sym_push(tok | SYM_STRUCT, &c, 0, -1);
            //   s->r = 0; /* default alignment is zero as gcc */
            //   push_prefix(get_tok_str(tok, NULL)); // class name
            //   next();
            //   skip('{');
            //   decl0(VT_CONST, 0, NULL);
            //   skip('}');
            //   pop_prefix();
            //   break;
            // } else {
            //   // anon prefix
            //   push_prefix("<anonymous>");
            //   skip('{');
            //   decl(VT_CONST);
            //   skip('}');
            //   pop_prefix();
...
int resolve_symbol_if_needed(int * t, Sym ** s) {
    int p = tok;
    next();
    uint8_t old_ptr = tcc_state->hierarchy_stack_pointer;
    if (tok == '.' || tok == TOK_ARROW || tok == TOK_CDOUBLE) {
        int l = tok; // .
        next();
        int l2 = tok; // field
        if (tcc_state->hierarchy_stack_pointer == 0) {
            Sym * ps = sym_find(p);
            #ifdef SYM_DEBUG
            char buf[500];
            const char * b = get_tok_str(p, NULL);
            type_to_str(buf, sizeof(buf), &ps->type, b);
            printf("symbol '%s' is type '%s'\n", b, buf);
            #endif
            tcc_state->hierarchy_stack[tcc_state->hierarchy_stack_pointer++] = ps;
        }
        Sym * symbol = find_field(&tcc_state->hierarchy_stack[tcc_state->hierarchy_stack_pointer-1]->type, l2, 0);
        if (symbol) {
            tcc_state->hierarchy_stack[tcc_state->hierarchy_stack_pointer++] = symbol;
            #ifdef SYM_DEBUG
            char buf[500];
            const char * b = get_tok_str(l2, NULL);
            type_to_str(buf, sizeof(buf), &symbol->type, b);
            printf("symbol '%s' is type '%s'\n", b, buf);
            #endif
        } else {
            symbol = find_field(&tcc_state->hierarchy_stack[tcc_state->hierarchy_stack_pointer-1]->type, l2 | VT_FUNC, 0);
            if (symbol) {
                tcc_state->hierarchy_stack[tcc_state->hierarchy_stack_pointer++] = symbol;
                #ifdef SYM_DEBUG
                char buf[500];
                ...
                char buf[500];
                const char * b = get_tok_str(p, NULL);
                type_to_str(buf, sizeof(buf), &tcc_state->hierarchy_stack[tcc_state->hierarchy_stack_pointer-1]->type, b);
                tcc_error("symbol '%s' with type '%s' does not contain a field named '%s'\n", b, buf, get_tok_str(l2, NULL));
                tcc_state->hierarchy_stack_pointer = old_ptr;
                return 0;
            }
        }
        next();
        int tk = tok; // .
        if (tk == '.' || tk == TOK_ARROW || tk == TOK_CDOUBLE) {
            unget_tok(l2);
            if (resolve_symbol_if_needed(t, s) == 1) {
                tcc_state->hierarchy_stack_pointer = old_ptr;
                return 1;
            }
        } else if (tk == '(') {
            ...
            const char * tok_str = get_tok_str(l2, NULL);
            tcc_warning("hashing name '%s'", tok_str);
            cstr_cat(&cstr, ".", 1);
            cstr_cat(&cstr, tok_str, strlen(tok_str));
            cstr_cat(&cstr, "", 1); // append '\0'
            ...
            Sym * symbol1 = sym_find(p);
            char buf[500];
            const char * b = get_tok_str(p, NULL);
            type_to_str(buf, sizeof(buf), &symbol1->type, b);
            tcc_error("symbol '%s' with type '%s' does not contain a field named '%s'\n", b, buf, get_tok_str(l2, NULL));
        }
        unget_tok(l2);
        unget_tok(l);
...
special_math_val:
    ...
    tok_identifier:
        ...
        unget_tok(t);
        if (!resolve_symbol_if_needed(&t, &s)) {
          next();
        }
        r = s->r;
...
#endif
            if ((type.t & VT_BTYPE) == VT_FUNC) {
          	    if (needs_prefix()) v = prefix_tok(v);
...
found:
		    ...
		    sym->type = type;
                    ...
                    // typedef ... struct/class/union/enum; is impossible due to language spec,  typedef int struct; // expect struct name
                    sym = sym_find(v);
